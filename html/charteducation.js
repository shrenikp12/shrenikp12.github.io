...
getCombinedData().then(combinedData => {
  ...
  const svg = d3
    .select("#yeareducation-chart") // Replace with the ID of the container element in your HTML
    .append("svg")
  ...
  d3.select("#play-button") // Replace with the ID of the play button in your HTML
    .on("click", () => {
      ...
      const intervalDuration = 1000; // Adjust the duration between each date in milliseconds
