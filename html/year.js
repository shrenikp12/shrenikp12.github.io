...
const svg = d3
  .select("#year-chart") // Replace with the ID of the container element in your HTML
  .append("svg")
...
d3.select("#intent-button") // Replace with the ID of the play button in your HTML
  .on("click", () => {
...
animate();
  ...
  d3.select("#race-button") // Replace with the ID of the play button in your HTML
  .on("click", () => {
    d3.select("#yearChart").remove(); 
  ...
  if (currentDate <= endDate) {
    cDate = currentDate.toString().substring(4, 15);
    document.getElementById("demo").innerHTML = cDate;
    interval = setTimeout(animate, intervalDuration);
  }
}
